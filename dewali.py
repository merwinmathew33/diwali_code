...
def draw_lamp(x, y, size):
    ...
    t.begin_fill()
    t.setheading(270)
    t.circle(size, 180)  # Draw the base of the lamp
    t.end_fill()

    t.penup()
    t.setpos(x+100, y + size-30)
    t.pendown()
    t.pencolor('yellow')
    t.fillcolor('yellow')
    t.begin_fill()
    t.circle((size / 2)-3,-180)  # Draw the flame of the lamp
    t.end_fill()
    ...
    t.pendown()
    t.begin_fill()
    t.setheading(0)  # this will fill the colour in triangle shape(head).
    for count in range(3):
        t.lt(120)
        t.fd(60)
    t.end_fill()
    t.penup()
draw_lamp(400, -200, 70)
